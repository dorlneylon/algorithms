...
struct node: splay::reversible_node<node> {
  long long val, val_min, label_add, sum;
  
  node(long long v = 0) : splay::reversible_node<node>(), val(v), sum(v) { val_min = label_add = 0; }
  ...
  void update() {
    ...
    val_min = val;
	sum = val;
    for (int t = 0; t < 2; ++t) if (c[t]) { val_min = std::min(val_min, c[t]->val_min); sum += c[t]->sum; }
  }
};
...
int main() {
  ...
  while (m--) {
	  ...
	  if (x == 1) {
		  ...
		  t.find_range(l - 1, r)->add(v);
	  } else if (x == 2) {
		  t.find_range(l - 1, r)->reverse();
	  } else if (x == 3) {
		  std::cout << t.find_range(l - 1, r)->val_min << "\n";
	  } else if (x == 4) {
		  std::cout << t.find_range(l - 1, r)->sum << "\n";
	  }
  }
}
